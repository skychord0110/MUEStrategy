...
### kabuステーションへのリクエストは直列化される

`KabuClient` は複数スレッド（PUSH処理・自動売買・口座スナップショット・定期買い集め検知）
から使われる。kabuステーションは同時リクエストに弱く、実測（2026-08-14 13:30）では
発注中に別スレッドが `/wallet/cash` を叩いた結果、
`/sendorder` が13.6秒かけて500、`/wallet/cash` が10秒でタイムアウト、
WebSocketが強制切断（WinError 10054）、が同時に起きた。
仕様上、流量超過なら429が返るはずなのでこれは流量制限ではない。

そのため `KabuClient` は内部ロックで**1リクエストずつ順番に**投げる。

口座スナップショットが見送るのは**発注・取消が進行中のときだけ**
（`client.order_in_flight`）。歩み値ポーリングのような参照系の後ろには普通に並ぶ。
当初は「誰かがリクエスト中か」（`client.busy`）で見送っていたが、
z値検知が50銘柄を1秒間隔で回し続けるため常時どれかが飛んでおり、
**35%（551/1588回・2026-08-17）が待つ必要のない相手を避けて空振り**していた。

`order_in_flight` はロックの順番待ち中も真になる。
待っている発注をスナップショットが追い越さないようにするため。

書き込み自体も、OneDrive配下ではウイルス対策や同期にファイルを掴まれて
`PermissionError`（WinError 5）で弾かれることがある（実測 2026-08-17 に1回）。
0.3秒→0.6秒と間隔を伸ばしながら最大3回やり直す。
3回とも失敗した場合だけログに出し、書きかけの一時ファイルは片づける。
