...
**実際に約定したときだけ**音が鳴る。検知（UNDER急増など）では鳴らさない
（検知は1日100件近く出るため、鳴らすと通知として機能しない）。

| 場面 | 音 |
|---|---|
| 新規約定（建玉成立） | ライフル 2.4秒 |
| 決済・利確（損益プラス） | ファンファーレ 2.2秒 |
| 決済・損切り（損益マイナス） | 沈む下降音 1.8秒 |

Windowsのポップアップ通知（plyer）は古いバルーン通知APIを使っており、
**Windows 11では音が鳴らない**（実測: サウンドスキームも再生デバイスも正常なのに
一度も鳴らなかった）。そのため `sounds/` のWAVを `winsound.PlaySound` で直接鳴らしている。

音源は標準ライブラリだけで合成したもの。生成と試聴はこの2つで行う。

```powershell
python ui\make_sounds.py            # 音源を生成・再生成
python ui\sound_preview.py set      # 新規約定→利確→損切 の順に試聴
```

差し替えは `config.yaml` の `notification.sound_files` にファイル名を書くだけ。
音源が見つからない場合は合成ビープに自動で切り替わる（無音にはならない）。
...
- `symbol_master.json` … 銘柄の売買単位・呼値グループのキャッシュ（当日ぶん）。
  `/symbol` は**1件あたり約790msかかり50銘柄で38秒**。これが起動時間のほぼ全てを
  占めていたため、同じ日のうちは取り直さない。日付が変わったら取り直す
  （株式併合などで売買単位が変わると発注数量を誤るため）。
  取得は起動をブロックせずバックグラウンドで進み、まだ取れていない銘柄に
  シグナルが来た場合は発注直前に1件だけ取りに行く。
