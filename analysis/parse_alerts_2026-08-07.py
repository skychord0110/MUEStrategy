# -*- coding: utf-8 -*-
"""runnerログ（2026-07-13〜08-07の全営業日）から全アラートをパースしCSV化する。

week1:07-13〜17 / week2:07-21〜24 / week3:07-27〜31 / week4:08-03〜07（計19営業日）
基礎検知アラートと、AI仮想売買（実稼働のフォワード実績）を別ファイルに出力する。

出力: analysis/output/alerts_2026-08-07.csv, paper_trades_2026-08-07.csv
実行: python -X utf8 parse_alerts_2026-08-07.py
"""
import re
import csv
import glob
...
with open(os.path.join(OUT_DIR, "alerts_2026-08-07.csv"), "w", newline="", encoding="utf-8") as f:
    w = csv.DictWriter(f, fieldnames=list(alerts[0].keys())); w.writeheader(); w.writerows(alerts)
with open(os.path.join(OUT_DIR, "paper_trades_2026-08-07.csv"), "w", newline="", encoding="utf-8") as f:
